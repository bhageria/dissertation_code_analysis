...
for event_generator in event_generators:
    applicationSource.write("\txTaskCreate("+event_generator +"_event_Handler, "+ "\""+ event_generator+"_Handler\", "+event_generator+"_STACK_SIZE"+", NULL, "+event_generator+"_PRIORITY"+", NULL);\n")
...
for module in event_generators:
    ...
    for sub_module in installModules:
        applicationSource.write("\tstr_"+sub_module+"_event_t " + sub_module + "_msg;\n")
    applicationSource.write("\twhile(1)\n\t{\n")
    applicationSource.write("\t\twhile(!xQueueReceive(q_"+module+"_event,&"+module+"_msg,portMAX_DELAY));\n")
...
for sequence_num in range(len(sequence)):
    ...
    for count in range(sequence[sequence_num]["number_of_actions"]):
        ...
        if(action_on == 'MQTT'):
            ...
            for module in event_generators:
                applicationSource.write("\t\t\t\tmqttData_e.data = "+ module + "_msg.data"+ ";\n")
            applicationSource.write("\t\t\t\txQueueSend(mqttData_Queue, &mqttData_e , (TickType_t)0 );\n")
        else:
            action_name = sequence[sequence_num]["action"][count]["action"]
            applicationSource.write("\t\t\t\t"+action_on + "_msg.msg_id = " +action_name +";\n")
            applicationSource.write("\t\t\t\txQueueSend(q_"+action_on + "_event, &"+action_on+"_msg, (TickType_t)0 );\n")
...
cmakefile.write("idf_component_register(SRCS  \"src/OTA.c\" \"main.c\" \"src/app_nvs.c\" \"src/app_wifi.c\" \"src/aws_iot.c\" \"src/LED.c\" \"system.c\" \"application.c\" ")
